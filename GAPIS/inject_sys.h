...
int inject_sys(const char *pkg , const char *module_name , const char *offset, void *target){
    ...
    void* hookc = (void *) syscall(222, NULL, sizeof(target), PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    memcpy(hookc, target, sizeof(target));
    fseek(fp, addr, SEEK_SET);
    fwrite(hookc, sizeof(target), 1, fp);
    fclose(fp);
    syscall(215,hookc, sizeof(target));
